...
places for status and rights logic. Tests first, then code, then `pnpm gates` from the root; name
the commit it ran on and paste the real tail once. Use absolute paths in shell commands. Stay inside the allowed files.

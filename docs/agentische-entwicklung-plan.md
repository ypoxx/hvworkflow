...
| Vertrag ist Quelle (Versions- und Changelog-Pflicht) | Vertragstor `packages/contract/scripts/check.mjs` (Scheibe 019; läuft als `test` des Vertragspakets in `pnpm -r test`; Prüfung (c) läuft in CI, seit `gates.yml` mit `fetch-depth: 0` auscheckt (012); dass ein fehlender Basisstand rot statt übersprungen ist, setzt 016 mit `CONTRACT_GATE_STRICT=1`) | eine Vertragsänderung ohne Versions- und Changelog-Eintrag; eine abgelaufene oder fehlerhafte Allowlist-Zeile | läuft (CI: Contract lint, typecheck, lint, unit tests, vocabulary, architecture, role-literals, now-check, plan-honesty, build) |

...
| PreToolUse auf Shell (voller Umfang) | blockiert ein bare `git push` (ohne explizites Remote und Branch — auch wenn ein Optionswert wie `--push-o <wert>` das Ziel vortäuscht), einen Force-Flag (`-f`, `--force`, `--force-with-lease`, auch abgekürzt wie `--force-w`, auch in einer kombinierten Kurzoption mit Ziffern wie `-4f`/`-f4`/`-6uf`), einen `+`-prefixed Refspec (auch gequotet), `--mirror`, einen Remote-Branch-Löschvorgang (`--delete`/`-d` oder `:` prefixed, auch abgekürzt oder gequotet), `--prune` mit Wildcard-Refspec, jeden Zugriff auf `.env*` außer `.env.example`, und erkennt `push` auch hinter einer beliebigen globalen Option zwischen `git` und dem Unterbefehl (nicht mehr nur einer festen Liste wie `-C`/`-c`/`--git-dir`). Die Argumente nach `push` werden als Shell-Wörter (Anführungszeichen und Escapes aufgelöst) gegen die Optionstabelle von `git push` (git 2.43) ausgewertet: Kurzoptionsgruppen (`-vd`, `-4f`, `-ofoo`), eindeutige Abkürzungen (`--pru`, `--dele`), `--no-`-Formen und Optionen mit Wert; eine mehrdeutige Abkürzung (`--forc`) wird blockiert, git lehnt sie ohnehin ab. Blockiert außerdem `curl`/`wget`, wenn der Befehl (als erstes Wort eines Abschnitts, getrennt an `&&`, einem Oder-Zeichen-Paar, einem Semikolon, einem einzelnen `&`, einer Shell-Pipe oder einem Zeilenumbruch — `curl`/`wget` als Ziel einer Shell-Pipe wird also erkannt, z. B. `echo x` weitergereicht an `curl https://…`) eine explizite `http(s)://`-URL nennt (Groß-/Kleinschreibung des Schemas egal) zu einem Host außer localhost/127.0.0.1/[::1]/0.0.0.0 (Host über `new URL()` bestimmt, damit eine geklammerte IPv6-Adresse nicht am ersten Doppelpunkt abschneidet) — erkennt dagegen **nicht** einen schema-losen Aufruf (`curl example.com`), `curl`/`wget` als Argument eines anderen Befehls (`xargs curl`, ein Shell-Alias/eine Funktion namens `curl`), eine Präfix- oder Pfadform (`FOO=1 curl`, `sudo curl`, `command curl`, `/usr/bin/curl`, `(curl …)`), oder einen anderen Netzzugriff aus einer Zeile (`node -e "fetch(...)"`, `python3 -c "import urllib.request; ..."`) — das ist ein Textscan, kein Shell-Parser, und keine Netzwerkrichtlinie (Codex-Review PR #17, verfeinert nach der Nacharbeitsrunde zu PR #20) | läuft (Hook: PreToolUse) |

...
| Vertrag ist Quelle (Versions- und Changelog-Pflicht) | Vertragstor `packages/contract/scripts/check.mjs` (Scheibe 019; läuft als `test` des Vertragspakets in `pnpm -r test`; Prüfung (c) wird im flachen CI-Checkout übersprungen, bis 016 `fetch-depth: 0` setzt) | eine Vertragsänderung ohne Versions- und Changelog-Eintrag; eine abgelaufene oder fehlerhafte Allowlist-Zeile | läuft (CI: Contract lint, typecheck, lint, unit tests, vocabulary, architecture, role-literals, now-check, plan-honesty, build) |

...
| PreToolUse auf Shell (voller Umfang) | blockiert ein bare `git push` (ohne explizites Remote und Branch — auch wenn ein Optionswert wie `--push-option <wert>` das Ziel vortäuscht), einen Force-Flag (`-f`, `--force`, `--force-with-lease`, auch abgekürzt wie `--forc` oder in einer kombinierten Kurzoption wie `-uf`), einen `+`-prefixed Refspec (auch gequotet), `--mirror`, einen Remote-Branch-Löschvorgang (`--delete`/`-d` oder `:` prefixed, auch abgekürzt oder gequotet), `--prune` mit Wildcard-Refspec, globale Optionen vor dem Unterbefehl (`-C`, `-c`, `--git-dir`, `--work-tree`, `--no-pager`), sowie jeden Zugriff auf `.env*` außer `.env.example`. Blockiert außerdem `curl`/`wget`, wenn der Befehl (als erstes Wort eines Top-Level-Shell-Abschnitts, getrennt an einem Und-Zeichen-Paar, einem Semikolon oder einer Shell-Pipe) eine explizite `http(s)://`-URL zu einem Host außer localhost/127.0.0.1/[::1]/0.0.0.0 nennt (Host über `new URL()` bestimmt, damit eine geklammerte IPv6-Adresse nicht am ersten Doppelpunkt abschneidet) — erkennt dagegen **nicht** einen schema-losen Aufruf (`curl example.com`), `curl`/`wget` hinter einer Shell-Pipe oder einem Wrapper (etwa `xargs curl`, ein Shell-Alias/eine Funktion namens `curl`), oder einen anderen Netzzugriff aus einer Zeile (`node -e "fetch(...)"`, `python3 -c "import urllib.request; ..."`) — das ist ein Textscan, kein Shell-Parser, und keine Netzwerkrichtlinie (Codex-Review PR #17) | läuft (Hook: PreToolUse) |

...
```
SC-03 nein — T-G1-I-01: listQuestions liest ohne can() (packages/domain/src/api.ts:397-402) → Blocker
```
...
| SP-7 Kill-Switch | Wirkt die Sperrliste von Subject-IDs ohne Neustart auf neue Anfragen, offene Sitzungen und offene SSE-Ströme? | Test „gesperrtes Subject → 401" (029) bleibt grün; neue Ströme oder Hintergrundjobs prüfen die Sperrliste | ein Weg (Strom, Webhook, Job), der die Sperrliste nicht kennt; ein Strom, der nach Abmelden oder Sperre offen bleibt oder bei Wiederaufnahme über eine Rechtegrenze abspielt | T-G1-S-02, T-G1-I-08, T-G1-I-09 |

## C. Ergänzende Prüfpunkte aus dem Bedrohungsmodell

| Punkt | Prüffrage | Wie prüfen | sichert ab |
|---|---|---|---|
| SC-07 Log-Integrität | Werden Ereignisse nur angehängt, aus benannten Feldern gebaut und mit der Zeit des Dienstes versehen? | kein `{ ...input }` oder `{ ...body }` als Payload: `grep -rn -e '\.\.\.input' -e '\.\.\.body' packages/domain/src apps/api/src`; keine Uhr außerhalb des Clock-Ports (seit 012 prüft das `pnpm now-check` für Domäne und Dienst): `grep -rn -e 'Date.now()' -e 'new Date()' packages/domain/src apps/api/src`; ab 024 Umschlagfelder gefüllt | T-G1-T-02, T-G1-T-07, T-G2-T-01, T-G2-S-02 |
...
| SC-10 Lieferkette | Hat eine neue Abhängigkeit Nutzen, Pflege und passende Lizenz (Leitplanken 6.12), steht sie im Lockfile, und sind Workflow-Änderungen minimal berechtigt und auf Commit-Hash gepinnt? | Diff von `package.json`, `pnpm-lock.yaml`, `.github/**`; `pnpm audit:check` grün, neue Ausnahme nur mit Grund, Eigentümer und Ablauf (seit 012); neuer Workflow-Baustein nur per Commit-Hash, `permissions:` nicht erweitert | T-Q-T-01, T-Q-T-02, T-Q-E-02 |

...
> werden und weil Beschäftigtendaten mitbestimmungsrelevant sein können (zu prüfen, E13).
...
| Auftragsverarbeiter Identität | Konzern-IdP (OIDC; Einordnung als Auftragsverarbeiter oder gemeinsam Verantwortlicher zu klären, Abschnitt 5); im Rückfall der Plattformbetreiber, der den Keycloak-Container betreibt (E10, E39) | Register E11, E38 |
...
| Erfassung (`capture`) | Redebeiträge, Einzelfragen | Erfassen und zerlegen. Sie verarbeitet den Namen der redenden Person bei der Aufnahme und bei der Zuordnung des Redebeitrags zur Wortmeldung (Eingabe, Ist-Analyse Phase P1); sie hält kein `question.identity.reveal` (Plan 3: Koordination, Versammlungsbüro, Recht, Freigabe, Podium). Ob sie in späteren Ansichten Klarnamen sieht, ist offen (Register, Zeile o. Nr. 1) |
...
| Beobachter (`observer`) | Zähler und Vorgelesenes (Plan B2, Scheibe 010) | kein Schreibrecht |
...
|---|---|---|---|
| Wortmeldung | Datensatz Wortmeldung mit Name, Organisation, Runde; `personId` verweist auf die Personentabelle | Versammlungsbüro, Koordination, Recht, Freigabe, Podium (Klarname); Erfassung bei Aufnahme und Zuordnung (Eingabe); Fachbereich (Pseudonym) | Aktionär |
...
| Historie | Alle Ereignisse, nur anhängend, mit `personId` statt Klarname | Recht, Freigabe (lesend); Beobachter nur Zähler und Vorgelesenes (Plan B2, Scheibe 010); Revision hat in der Beta kein Konto (E24); personenbezogene Auswertung nur mit zweiter Freigabe | Aktionär und Beschäftigte |
...
| Technisches Zugriffslog | requestId (Korrelations-ID), `subjectHash` (gehashtes Subject, kein Klartext; Plan Scheibe 033), operationId, Status, Latenz, seq; **nie Fragetext, keine Nutzdaten** | 30 Tage (konfigurierbar); Klasse `technical`; getrennte Senke; die Aufbewahrung des `subjectHash` ist in 033 im DSFA-Entwurf zu beschreiben | nur zu zweit; keine personenbezogene Leistungsauswertung; Rate-Limit-Zähler flüchtig |
...
| V9 | Vorgangshistorie führen | Alle Ereignisse mit personId, Zeit, Hash-Kette | Aktionär und Beschäftigte | Nachweis für Niederschrift und Anfechtungsverfahren; Revisionssicherheit | Art. 6 Abs. 1 lit. c i. V. m. §§ 130, 243 ff. AktG; ggf. lit. f (Rechtsverteidigung) — **zu prüfen** | Recht, Freigabe (lesend); Beobachter nur Zähler und Vorgelesenes (Plan B2, Scheibe 010); Notar (Export); Revision ohne Konto in der Beta (E24) | record; unbegrenzt bis E16 | Nur anhängend (Regel 7); personenbezogene Auswertung nur mit zweiter Freigabe (047); Legal Hold als Attribut (024) |
| V10 | Technisches Zugriffslog | requestId, `subjectHash` (gehashtes Subject, Plan Scheibe 033), operationId, Status, Latenz, seq, Zeit; nie Fragetext | Beschäftigte | Sicherheit, Störungsanalyse, Missbrauchserkennung | Art. 6 Abs. 1 lit. f DSGVO; § 26 BDSG bzw. Nachfolgeregelung — **zu prüfen** | Betrieb (nur zu zweit) | technical; 30 Tage (konfigurierbar); Aufbewahrung des `subjectHash` in 033 zu beschreiben | Getrennte Senke, keine Nutzdaten, Auswertung zu zweit, keine Leistungsauswertung (033) |
...
| V12 | Rollenzuordnung je Jahrgang | subject → Rollen, optional Einheit, Ablaufdatum; Ereignisse RoleAssigned/RoleRevoked | Beschäftigte | Berechtigungsverwaltung, Nachweis „wer durfte was am HV-Tag" | wie V11 — **zu prüfen** | Administration; Revision ohne Konto in der Beta (E24) | record | Ablauf am Jahrgangsende; eingefrorener Rechte-Snapshot je Jahrgang (026, 077) |
| V13 | Klarnamen auflösen | Ereignis: wer hat wann welchen Klarnamen zu welcher Frage aufgelöst | Aktionär und Beschäftigte | Beantwortung durch berechtigte Rollen; Bühne | wie V1; Protokollierung: Art. 6 Abs. 1 lit. f (Nachweis) — **zu prüfen** | Inhaber von `question.identity.reveal` | record | Eigenes Recht, jede Auflösung protokolliert (Recht aus 026, Protokoll aus 067) |
...
| V18 | Löschung des Übungsbestands | gesamte Datenbank des Übungsmandanten | alle Gruppen der Probe | Zweckwegfall nach der Generalprobe | Keine Rechtsgrundlage einer Verarbeitung, sondern Pflicht: Speicherbegrenzung Art. 5 Abs. 1 lit. e DSGVO und Löschpflicht bei Zweckwegfall Art. 17 Abs. 1 lit. a DSGVO (Art. 17 ist zugleich Betroffenenrecht); Nachweis über das Löschprotokoll, Art. 5 Abs. 2 — **zu prüfen** | Plattformbetreiber | — (Löschung) | Entfernen der getrennten Datenbank, nie der Dienstrolle; Löschprotokoll; Nachweis in der Go/No-Go-Liste (042, 078) |
